...
#define SERVER_PORT "21944" // SERVERA_PORT, backend server A port
#define MAXBUFLEN 100   // udp, only used this when receive request from serverM.
#define INPUT_FILE "block1.txt" // input txt file

using namespace std;

// reference Reej's bOOk, mostly on the UDP listener part.

// get sockaddr, IPv4 or IPv6:
void *get_in_addr_serverA(struct sockaddr *sa)
{
...
int main(void)
{
    ...
    cout<< "The serverA is up and running using UDP on port " << SERVER_PORT << "." << endl;
    ...
    while(1){
        ...
        else if(strcmp(buf, txlist_str) == 0) // buf == TXLIST)
        {
            ...
            while(getline(infile, one_line)) // !infile.eof()) // To get all lines.
            {
                ...
                // cout << "read a new line: " << original_line << endl;
        ...
        // if write, first 2 chars are "W ", then comes the log string.
            ...
            ofstream outfile;
            outfile.open(INPUT_FILE, ios::app); // ofstream::app); // append to file, do not overwrite
            char *first_2_chars_chopped = buf + 2;
            // cout << first_2_chars_chopped << endl;
        ...
        } // end if ask for a username and balance.

        cout << "The ServerA finished sending response to the Main Server." << endl;
